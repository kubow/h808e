<script>
  ...
  async function prepareSource(source) {
    ...
    if (source.source_address) {
      ...
      currentContent = await loadDataUrl($h808e.api.scrapestack.url.replace("BOBOK", $h808e.api.scrapestack.access_key)+source.url);
      console.log(currentContent);
...
{:then data_set}
    <div class="cards-list">
        {#each data_set as x}
          <Popover on:open={() => prepareSource(x)} on:close={() => console.log("popover closed")}>
            <div slot="target">
              <Btn ghost>
                <div class="card">
                    <div class="card_image">
                        <slot name="card_image">
                            ...
                            {:else}
                                <img src="https://i.redd.it/b3esnz5ra34y.jpg" alt="sample content"/>
                            {/if}
                        </slot>
                    </div>
