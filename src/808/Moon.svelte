<script>
	...
	var Moon = {
		phases: ['new-moon', 'waxing-crescent-moon', 'waxing-quarter-moon', 'waxing-gibbous-moon', 'full-moon', 'waning-gibbous-moon', 'last-quarter-moon', 'waning-crescent-moon'],
	...
	moon_phase = Moon.phase(date.getFullYear(), date.getMonth() + 1, date.getDate());
	//console.log(moon_phase);

	async function getAstroWeb(variant) {
		if (variant = 1) {
			let data = await loadDataUrl($h808e.api.ipgeoloc.url.replace("BOBOK", $h808e.api.ipgeoloc.access_key));
			//console.log(data);
			return data
		} else {
			var opt_string=[];
			...
			let data = await loadDataUrl('https://www.icalendar37.net/lunar/api/?'+opt_string.join("&"));
			return data
		}
	}
			...
			<div class="moonDisplay" style={constructBackground()}>
				<span class="moonTip">
					...
					{:then ast_info}
						...
						<p>moon alt: {roundTo(ast_info.moon_altitude, 2)} angle {roundTo(ast_info.moon_parallactic_angle, 2)}</p>
...
<style>
	section, .moonWrapper, .moonDisplay {
		display: flex;
