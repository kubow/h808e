<script>
	...
	let dateTable = calendar();
	let dayOrNight = 'AM';

	async function calendar() {
		let calContent = await loadDataJson('calendar.json');
		let result = []
		result.push("Je "+weekDay(date)+" / "+weekNumber(date)+". týden / "+dayNumber(date)+". den v roce");
		calContent.forEach(function(entry) {
			if (entry["`n_month`"] == month && entry["`n_day`"] == day) {
				if (entry["`calendar`"] == 'cz_jmena') {
					result.push('Dnes má svátek '+entry["`day_name`"])
				} else if (entry["`calendar`"] == 'cz_important') {
					result.push(entry["`day_name`"])
				} else if (entry["`calendar`"] == 'cz_holiday') {
					result.push(entry["`day_name`"])
				} else if (entry["`calendar`"] == 'cz_other') {
					result.push(entry["`day_name`"]+'('+entry["`day_memo`"]+')')
				}
			}
		});
		return result;
...
<section>
	<div class='clockWrapper'>
		<p class='clockDisplay'> 
			{day}. {month}. {year} <br/> {hour} : {min} : {sec} {dayOrNight}
			<span class="tooltip-wrapper">
				<span class="tooltip">
					{#await dateTable}
						<p>...data not yet loaded</p>
					{:then dateTable}
						{#each dateTable as important}
							<p>{important}</p>
						{/each}
					{:catch error}
